		...
		if(haved=="MyAdmin"):
			...
			for data in lst:
				...
				if((UsernameorEmail==data.EmailId or UsernameorEmail==data.UserId) and Password==data.Password):
					...
					values.save()
					return render(request,"promanager/index.html",{"pmID":values.id,"pmName":values.ProManagerName});
					#return redirect("/promanager/ActiveAdmin",{"pmID":values.id,"pmName":values.ProManagerName});
					#return redirect("/promanager",{"pmID":values.id,"pmName":values.ProManagerName});
					#return redirect("/promanager",{"pmID":"07","pmName":"Shivam Shukla"});
...
def logoutMethod(request):
	...
	#return redirect("promanager/logout/");
