...
class AssignProjectToDeveloperByProjectManager(models.Model):
	...
	ProjectId = models.CharField(max_length=10); 
	def __str__(self):
		return "ProManager '"+self.ProManagerName+"' assigned project '"+self.ProjectName+"', to developer '"+self.DeveloperName+"'.";
...
class ActiveAdmin(models.Model):
	...
	def __str__(self):
		return "ProManager '"+self.ProManagerName+"' "+self.Status+" now.  "+str(self.id);
