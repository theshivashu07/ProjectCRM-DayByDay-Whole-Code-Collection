...
def editsMethod(request):
	if request.method=="POST":
		...
		if(haved=="Developer" or haved=="ProjectManager"):
		...
		if(fromwhere=="Developer" or fromwhere=="ProjectManager"):
...
'''
data=RecruitDeveloper.objects.get(pk=vari);



		elif(fromwhere=="ProjectManager"):
			pmvalue=RecruitDeveloperAsProjectManager.objects.get(pk=request.GET["var1"]);
			devsvalue=RecruitDeveloper.objects.all();
			variable=int(pmvalue.DevsId)
			for var in devsvalue:
				if(variable==var.id):
					data=var;
					break;
'''
